...
impl MmapChoice {
    ...
    /// # Safety
    ///
    /// This constructor is unsafe because file backed memory maps are an
    /// inherently unsafe construct in Rust, and there is no obvious way to
    /// encapsulate them on all platforms without simultaneously negating some
    /// or all of their benefits. As such, enabling memory maps requires the
    /// caller to write `unsafe` code.
    ///
    ...
    pub(crate) fn open(
        ...
        // SAFETY: This unsafe is acceptable because the only way
        // `MmapChoiceImpl` can be `Auto` is if the caller invoked the unsafe
        // `auto` constructor. Thus, this `unsafe` is a propagation of the
        // caller's assertion that using memory maps is safe.
